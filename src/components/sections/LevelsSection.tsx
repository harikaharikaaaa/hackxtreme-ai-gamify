...
const LevelsSection = () => {
  return (
    <section id="levels" className="py-24 relative bg-cyber-darker">
            ...
            {levels.map((level, index) => (
              ...
              >
                <div className="flex flex-col items-center justify-start mb-8">
                  <div 
                    className={`w-8 h-8 md:w-10 md:h-10 rounded-full z-10 flex items-center justify-center
                      ${level.isCompleted ? 'bg-cyber-blue' : level.isActive ? 'bg-cyber-purple animate-pulse-glow' : 'bg-cyber-dark border border-cyber-grey'}`}
                  >
                    {level.isCompleted ? (
                      <CheckCircle className="w-5 h-5 text-cyber-darker" />
                    ) : level.isActive ? (
                      <Sparkles className="w-5 h-5 text-cyber-light" />
                    ) : (
                      <Lock className="w-4 h-4 text-cyber-grey" />
                    )}
                  </div>
                ...
                <div className={`cyber-card h-32 ${level.isActive ? 'neon-border-purple' : ''} transition-all duration-300 hover:-translate-y-2`}>
                  ...
                  {level.isActive && level.progress > 0 && (
                    <div className="mt-4">
                      ...
                      <p className="text-xs text-cyber-grey mt-1 text-right">{level.progress}% complete</p>
                    </div>
                  )}
                </div>
              </motion.div>
            ...
            <div className="cyber-terminal p-6">
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="neon-text-blue">1,250 / 3,000</span>
                </div>
                <div className="h-2 bg-cyber-dark/50 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-cyber-blue to-cyber-purple w-[42%]"></div>
                </div>
              </div>
